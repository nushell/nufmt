(
    ...
    [[a, b]; [one, 2], [one, 4], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr max)
    [[a, b]; [one, 2], [one, 4], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr mean)
    [[a, b]; [6, 2], [4, 2], [2, 2]] | dfr into-df | dfr mean
    [[a, b]; [6, 2], [4, 2], [2, 2]] | dfr into-df | dfr median
    [[a, b]; [one, 2], [one, 4], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr median)
    [[a, b, c, d]; [x, 1, 4, a], [y, 2, 5, b], [z, 3, 6, c]] | dfr into-df | dfr melt -c [b, c] -v [a, d]
    [[a, b]; [one, 2], [one, 4], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr min)
    ...
    [[a, b]; [6, 2], [1, 4], [4, 1]] | dfr into-lazy | dfr with-column (dfr when ((dfr col a) > 2) 4 | dfr otherwise 5 | dfr as c) | dfr with-column (
        dfr when ((dfr col a) > 5) 10 | dfr when ((dfr col a) < 2) 6 | dfr otherwise 0 | dfr as d
    ) | dfr collect
    [[a, b]; [one, 2], [one, 4], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr quantile 0.5)
    ...
    [[a, b]; [one, 2], [one, 2], [two, 1], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr std)
    ...
    [
        ...
        6
        6
    ] | dfr into-df | dfr value-counts
    [[a, b]; [6, 2], [4, 2], [2, 2]] | dfr into-df | dfr var
    [[a, b]; [one, 2], [one, 2], [two, 1], [two, 1]] | dfr into-df | dfr group-by a | dfr agg (dfr col b | dfr var)
    dfr when ((dfr col a) > 2) 4
    dfr when ((dfr col a) > 2) 4 | dfr when ((dfr col a) < 0) 6
    [[a, b]; [6, 2], [1, 4], [4, 1]] | dfr into-lazy | dfr with-column (dfr when ((dfr col a) > 2) 4 | dfr otherwise 5 | dfr as c) | dfr with-column (
        dfr when ((dfr col a) > 5) 10 | dfr when ((dfr col a) < 2) 6 | dfr otherwise 0 | dfr as d
    ) | dfr collect
    [[a, b]; [1, 2], [3, 4]] | dfr into-df | dfr with-column ([5, 6] | dfr into-df) --name c
    [[a, b]; [1, 2], [3, 4]] | dfr into-lazy | dfr with-column [
    ...
    error make {
        ...
        label: {text: "my custom label text", start: 123, end: 456}
    }
    def foo [x] {
    ...
    module utils {
        export def my-command [] { "hello" }
    }
    use utils my-command
    my-command
    ...
    use spam foo
    foo
    module foo { export def-env bar [] { let-env FOO_BAR = "BAZ" } }
    use foo bar
    bar
    $env.FOO_BAR
...
<note>
  <remember>Event</remember>
    ...
    def say-hi [] { echo 'Hi!' }
    hide say-hi
    let-env HZ_ENV_ABC = (1)
    hide-env HZ_ENV_ABC
    'HZ_ENV_ABC' in (env).name
    ls | histogram type
    ...
    [
        ...
        1
        1
    ] | histogram --percentage-type relative
    history | length
    history | last 5
    ...
    [1, 2, 3, 4, 5] | length
    [
        {columnA: A0, columnB: B0}
    ] | length -c
    let x = 10
    let x = 10 + 100
    let x = if false { -1 } else { 1 }
    let-env MY_ENV_VAR = (1)
    $env.MY_ENV_VAR
    $"two\nlines" | lines
    ...
    use spam foo
    foo
    module foo { export-env { let-env FOO = "BAZ" } }
    use foo
    $env.FOO
    module foo { export def-env bar [] { let-env FOO_BAR = "BAZ" } }
    use foo bar
    ...
    print (2 + 3)
    def spam [] { "spam" }
    profile {|| spam | str length } -d 2 --source
    ps
    ...
    http get https://en.wikipedia.org/wiki/List_of_cities_in_India_by_population | query web -t [Rank, City, 'Population(2011)[3]', 'Population(2001)', 'State or union territory']
    ...
    let plugin = ((which nu).path.0 | path dirname | path join 'nu_plugin_query')
    ...
    [
        {a: 'nutshell', b: 'numetal'}
    ] | str distance 'nushell' 'a' 'b'
    'NU' | str downcase
    'TESTa' | str downcase
        ...
        content: [
            {
                ...
                attributes: {}
                content: [
                    {tag: null, attrs: null, content: Event}
                ]
            }
        ]
    } | to xml
    ...
    [["2021-04-16", "2021-06-10", "2021-09-18", "2021-10-15", "2021-11-16", "2021-11-17", "2021-11-18"]; [37, 0, 0, 0, 37, 0, 0]] | update cells { |value|
          if $value == 0 {
            ""
          } else {
            $value
    ...
    [["2021-04-16", "2021-06-10", "2021-09-18", "2021-10-15", "2021-11-16", "2021-11-17", "2021-11-18"]; [37, 0, 0, 0, 37, 0, 0]] | update cells -c ["2021-11-18", "2021-11-17"] { |value|
            if $value == 0 {
              ""
            } else {
              $value
    ...
    'https://example.com/foo bar' | url encode --all
    {
        ...
        "fragment": ""
        "params": {"mode": "medium", "illust_id": "99260204"}
    } | url join
    {
        "scheme": "http"
        "username": "user"
    ...
    def-env foo [] { let-env BAR = 'BAZ' }
    view source foo
    module mod-foo { export-env { let-env FOO_ENV = 'BAZ' } }
    view source mod-foo
    alias hello = echo hi
    view source hello
    some | pipeline | or | variable | debug -r
    view span 1 2
