...
# Test result record
def make_result [name: string, passed: bool, message: string] { {name: $name, passed: $passed, message: $message} }
...
def run_test [name: string] {
    ...
    if not ($input_file | path exists) { return (make_result $name false $"Input file not found: ($input_file)") }
    if not ($expected_file | path exists) { return (make_result $name false $"Expected file not found: ($expected_file)") }
    # Read input
    let input = open $input_file
    # Run formatter
    let result = try {
        $input | ^$NUFMT_BINARY --stdin
    } catch { return (make_result $name false $"Formatter error: ($err.msg)") }
    ...
    if $formatted_normalized == $expected_normalized { make_result $name true } else {
...
def run_idempotency_test [name: string] {
    let input_file = $"($INPUT_DIR)/($name).nu"
    if not ($input_file | path exists) { return (make_result $"($name)_idempotency" false $"Input file not found") }
    let input = open $input_file
    # First format
    let first = try {
        $input | ^$NUFMT_BINARY --stdin
    } catch { return (make_result $"($name)_idempotency" false "First format failed") }
    # Second format
    let second = try {
        $first | ^$NUFMT_BINARY --stdin
    } catch { return (make_result $"($name)_idempotency" false "Second format failed") }
    if ($first | str trim) == ($second | str trim) { make_result $"($name)_idempotency" true } else { make_result $"($name)_idempotency" false "Output changed on second format" }
}
# Get all test names from input directory
def get_test_names [] {
    ls $INPUT_DIR | where name =~ '\.nu$' | get name | each {|f| $f | path basename | str replace '.nu' ''}
}
...
def main [
    ...
    # Only run idempotency tests
    # Only run ground truth tests
    ...
    if $list {
        ...
        for category in ($TEST_CONSTRUCTS | columns) {
            ...
            for name in ($TEST_CONSTRUCTS | get $category) {
                ...
                let status = if $input_exists and $expected_exists { green "✓" # missing expected } else { red "✗" # missing input } } else if $input_exists {
                    yellow "○"
                    print $"  ($status) ($name)"
                }
            }
        ...
        if $check_files {
            ...
            for test in $defined_tests {
                if not ($"($INPUT_DIR)/($test).nu" | path exists) { $missing_inputs = ($missing_inputs | append $test) }
                if not ($"($EXPECTED_DIR)/($test).nu" | path exists) { $missing_expected = ($missing_expected | append $test) }
            }
            for test in $existing_inputs {
                if not ($test in $defined_tests) { $undefined_tests = ($undefined_tests | append $test) }
            }
            ...
            if ($missing_inputs | length) == 0 and ($missing_expected | length) == 0 { print (green "All defined tests have both input and expected files!") }
            return
        }
        # Determine which tests to run
        let tests_to_run = if $test != null { [$test] } else if $category != null { get_tests_by_category $category } else { get_all_defined_tests }
        ...
        if not $idempotency {
            ...
            for name in $tests_to_run {
                ...
                $results = ($results | append $result)
                if $result.passed { print $"  (green '✓') ($name)" } else {
                    print $"  (red '✗') ($name)"
        ...
        if not $ground_truth {
            ...
            for name in $tests_to_run {
                ...
                if not ($"($INPUT_DIR)/($name).nu" | path exists # Skip silently for idempotency if no input }) {
                    continue
                    let result = run_idempotency_test $name
                    $results = ($results | append $result)
                    if $result.passed { print $"  (green '✓') ($result.name)" } else {
            ...
            if $failed > 0 {
                ...
                for result in ($results | where {|r| not $r.passed}) {
                    print $"  - ($result.name)"
                    if $verbose and ($result.message | str length) > 0 { print $"      ($result.message | str substring 0..200)..." }
                }
                exit 1
            }
            print ""
